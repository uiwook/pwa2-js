...
// `?.`을 기준으로 앞의 평가대상이 undefined || null 이 아니면 처리를 계속 이거가고
// undefined || null이면 false를 반환하고 멈춘다.
// 앞에꺼?.뒤에꺼?.그뒤에꺼 -> 앞에꺼 있으면 뒤에꺼 찾아간다.
// 뒤에꺼 있으면 그뒤에꺼 찾아간다.
...
// -------------------------------------
// Array(배열) 객체
// 하나의 변수에 여러개의 값을 순차적으로 저장(iterable)할 수 있는 데이터 구조
// 여러 종류의 데이터 타입을 저장 가능(자료구조론에서는 하나만 가능)
// 배열의 길이는 동적으로 변화(자료구조론에서는 변화하지 않음)
...
// concat(...args) : 배열의 마지막 요소를 추가한 새로운 배열을 반환(딥 카피)
const arr5 = [1, 2, 3];
...
console.log(resultShift, arr7); 

// splice(start, count, ...args) : 원본에서 특정 요소를 자르다.
...
// start만 지정했을 때
  ...
  //start가 음수일 경우, 배열의 끝 요소부터 start만큼의 요소를 제거하고 잘린 배열을 반환
...
// (a - b)가 0일 경우, a가 작은수, 같은 값으로 인식하여 정렬하지 않음

const arr13 = [5, 23, 24, 27, 16, 99, 532, 2342];
...
// forEach(cb) : 배열의 모든 요소에 대해서 콜백함수를 반복 실행
